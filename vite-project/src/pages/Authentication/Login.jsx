...
import { auth, googleProvider, appleProvider, signInWithPopup, signInWithEmailAndPassword, sendPasswordResetEmail } from './firebaseConfig.jsx';
          ...
          <div className='bg-black min-[320px]:h-full sm:w-full sm:h-fit lg:h-max md:h-fit lg:w-full'>
            ...
            <div className='flex max-w-screen min-[320px] flex-wrap-reverse place-content-center md:flex-wrap-reverse lg:flex-nowrap'>
              <div className='flex-row min-[320px] px-12 sm:px-28 mt-10 2xl:mt-44 mb-5'>
                ...
                <Form>
                  <div className='flex flex-col gap-y-2 text-zinc-400'>
                    ...
                    <div className='flex justify-between'>
                      ...
                      <button type="button" onClick={() => setIsResetRequested(true)} className='text-white text-sm'>
                        Forgot Password?
                      </button>
                    </div>
                ...
                {isResetRequested && (
                  <div className='mt-4'>
                    <label htmlFor="resetEmail" className='text-zinc-400'>Enter your email to reset password:</label>
                    <input
                      type="email"
                      id="resetEmail"
                      value={resetEmail}
                      onChange={(e) => setResetEmail(e.target.value)}
                      className='min-[320px] h-9 md:w-72 lg:w-72 bg-zinc-900 rounded-lg p-3 mt-2'
                    />
                    <button onClick={handlePasswordReset} className='bg-white text-black rounded-md h-10 gap-34 font-bold hover:bg-zinc-200 border-solid border-2 hover:border-zinc-700 mt-2'>
                      Send Reset Email
                    </button>
                  </div>
                )}
